...
var yAxis = 20; /** represents the Y position of the upper left corner of the Scout View Canvas window. doesnt change **/
...
$("#notvisible").prop("disabled", true);
$("#mixed").prop("disabled", true);
$("#randomImgs").prop("disabled", true);
...
function clearRadioLL() {
	var ele = document.getElementsByName("lat");
	var ele2 = document.getElementsByName("land");
...
function clearRadioMix() {
	var ele = document.getElementsByName("mixedr");
	   for(var i=0;i<ele.length;i++) {
	      ele[i].checked = false;
	  }
}
...
$("#notvisible").prop("disabled", false);
$("#mixed").prop("disabled", false);
$("#randomImgs").prop("disabled", false);
}
...
function drawLine(y) {
  ...
  refline.font = "12px Arial";
  refline.fillStyle = '#66ff00';
  refline.fillText("110 Slices, 9.020mm, 2.8min", 190, 508);
...
function nextImage() {
  ...
  if (countSet > 8) { //what happens when you reach the end of the set
    ...
    wrapSet = wrapSet + 1;
    if (wrapSet == 3) {
    	curSetNum = curSetNum - 2;
    	wrapSet = 0;
    	alert("helewrwerlo");
    } else {
    	curSetNum = curSetNum + 1;
    } if (onRandom == 1) {
    	curSetNum = 'random';
    }
    //alert(curSetNum);
